...
public final class WorkCardProtocol {
    ...
    public static List<Frame> decode(List<Byte> buffer) {
        ...
        while (true) {
            ...
            int expectedCrc = unsigned(raw[totalLength - 2]) << 8 | unsigned(raw[totalLength - 1]);
            int actualCrc = crc16Modbus(raw, 0, totalLength - 2);
            if (expectedCrc != actualCrc) { buffer.remove(0); continue; }
