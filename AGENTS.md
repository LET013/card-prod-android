...
- `app/src/main/java/com/xingyao/card/service/DeviceCoreService.java`：设备业务编排入口。
...
- `app/src/main/java/com/xingyao/card/JsBridge.java`：受信来源桥接与原生权限边界。
- `uniapp/src`：H5 UI、交互和原生桥接调用。

## 修改流程

1. 明确本批唯一目标和不在范围内的内容。
2. 画出入口 → 业务层 → 通信层 → Repository → UI/后台上报的调用链。
...
- 标记任何把传输成功误当成业务成功的代码。
- 标记任何绕过 Repository、直接让 UI 推导卡槽状态的代码。
