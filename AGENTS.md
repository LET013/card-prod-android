...
- 不再为每个子项新建独立分支；
- 不再创建层层依赖的堆叠 PR；
- 所有已确认的前端/客户端修改按顺序提交到统一分支；
- 每个子项仍使用独立 `docs/plans/<task-id>/` 目录和独立提交；
- 设计阶段和实现阶段在同一个 Draft PR 中分阶段进行；
- 设计提交后必须等待用户确认，确认前不得继续写运行代码；
- 需要隔离高风险实验时，只有用户明确授权才能临时创建额外分支；
- 统一分支始终以 `reference/motone-current` 为目标基线，不修改 `main`。
...
高风险任务必须在统一分支和同一个 Draft PR 中执行“设计阶段 → 用户确认 → 实现阶段”，不得跳过设计确认，也不得通过新建堆叠分支绕开门禁。
...
- 前端与客户端统一在 `feature/client-integration` 分支持续工作；
- 每个子项使用独立计划目录和独立提交，不使用独立功能分支；
- 同一个 Draft PR 中依次进行设计审查和实现审查；
- 用户未确认设计时，统一分支只允许提交该子项的文档；
- 用户确认后，才允许继续向同一分支提交该子项运行代码；
- 默认 Draft PR，不自动合并；
- 禁止修改 `main`；
