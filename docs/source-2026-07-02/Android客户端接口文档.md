...
- **MQTT模式**: 连接EMQX → 订阅 `card/{deviceCode}/down` 和 `card/{deviceCode}/down/response` → 发送 `login` 上行消息
- **HTTP模式**: `POST /api/v1/device/login`
...
**data**: `{ "cardNo": "CARD001", "eventType": "TAKE", "slotId": 1, "authType": "FACE", "timestamp": 1753001234567 }`

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| cardNo | String | 是 | 卡号 |
| eventType | String | 是 | `TAKE`(取卡) / `RETURN`(还卡) |
...
| 取还卡事件 | `cardEvent` (up) | `POST /api/v1/card/event` |
