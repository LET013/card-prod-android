...
JNIEXPORT jobject JNICALL Java_com_xingyao_card_serial_SerialPort_open
    ...
    /* Configure device */
    {
        ...
        // 清除 O_NONBLOCK，使 fd 变为阻塞模式（FileInputStream.read() 才能阻塞等待数据）
        {
    ...
    /* Store into SerialPort.mFd so closeNative() can use it */
    {
...
JNIEXPORT void JNICALL Java_com_xingyao_card_serial_SerialPort_closeNative
    ...
    if (mFd == NULL) {
        LOGD("closeNative() called but mFd is null, already closed?");
        return;
    }
