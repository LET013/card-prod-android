<template>
  <ModalShell closable @close="$emit('close')">
    <view class="password-content">
      ...
      <text class="password-help">{{ submitting ? '正在验证，请稍候…' : help }}</text>
...
defineProps({
  title: { type: String, default: '请输入管理密码' },
  help: { type: String, default: '输入您的密码进行登陆，设备管理页面' }
})
const password = ref('')
const error = ref('')
...
const submit = () => {
  if (password.value.length !== 6 || submitting.value) return
...
.person-icon .head { width: 32%; height: 32%; border-radius: 50%; background: #1f76ff; position: absolute; left: 34%; top: 21%; }
...
.password-help { margin-top: 22px; text-align: center; font-size: clamp(18px, 2.5vw, 23px); font-weight: 400; line-height: 1.5; color: #8290a3; }
