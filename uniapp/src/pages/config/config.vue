  ...
  <view class="page-root config-page">
    <view class="config-scroll">
      <view class="config-content">
        ...
        <text class="server-help">*请设置自己的服务器IP，填写示例：http://192.168.0.153:808</text>
...
const validate=()=>{
  ...
  if(!String(form.serverAddress).trim()) return '服务器IP不能为空'
...
const save=async()=>{
  ...
  saving.value=true
  try{await services.saveSettings({...form});Object.assign(appState.settings,form,{initialized:true});uni.showToast({title:'保存成功',icon:'success'});setTimeout(()=>uni.reLaunch({url:'/pages/index/index'}),450)}catch(error){uni.showToast({title:error.message||'保存失败',icon:'none'})}finally{saving.value=false}
}
const goBack=()=>{
