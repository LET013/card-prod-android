...
const activateEngine=async()=>{engineActivating.value=true;appState.runtime.recognitionEngine={...appState.runtime.recognitionEngine,state:'ACTIVATING',message:'正在重新激活 FaceAISDK'};try{await services.reactivateFaceEngine();await services.getRuntime();setTimeout(()=>services.getRuntime().catch(()=>{}),1500);uni.showToast({title:'已发送重新激活指令',icon:'none'})}catch(e){uni.showToast({title:e.message||'重新激活失败',icon:'none'})}finally{engineActivating.value=false}}
