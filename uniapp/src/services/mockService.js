...
import { createSlots, defaultEmployees, upgradeFiles } from '@/mock/data.js'

const wait = (ms = 420) => new Promise((resolve) => setTimeout(resolve, ms))
const passwordRoleMap = { '111111': ROLE.SYSTEM_ADMIN, '222222': ROLE.OPS, '333333': ROLE.DEVELOPER }

const ensureMockData = () => {
  if (!appState.slots.length) {
    appState.slots.splice(0, appState.slots.length, ...createSlots(
      Number(appState.settings.totalCount) || 100,
      Number(appState.settings.singleGroupCount) || 10
    ))
  }
  if (!appState.employees.length) {
    appState.employees = JSON.parse(JSON.stringify(defaultEmployees))
  }
}

export const mockService = {
  wait,
  async loadSettings() { await wait(120); ensureMockData(); return { ...appState.settings } },
  ...
  async getRuntime() { await wait(180); return JSON.parse(JSON.stringify(appState.runtime)) },
  ...
  async logout() { appState.session = null; return true },
  async getSlots() { await wait(160); ensureMockData(); return JSON.parse(JSON.stringify(appState.slots)) },
  async unlockDoor(slotNumber) {
    await wait(520); ensureMockData()
    const slot = appState.slots.find((item) => item.slotNumber === Number(slotNumber))
    if (!slot) throw new Error('卡门不存在')
    slot.doorStatus = '已开锁'; slot.updatedAt = Date.now()
  ...
  async unlockAllDoors() { await wait(680); ensureMockData(); return { success: true, successCount: appState.slots.length, failedCount: 0, message: '全部卡门模拟开锁完成' } },
  async runRecognition(type, onProgress) {
    ensureMockData()
    for (const stage of (type === 'FACE' ? ['PREPARING', 'DETECTING', 'MATCHING'] : ['PREPARING', 'COLLECTING', 'MATCHING'])) {
      onProgress?.(stage); await wait(stage === 'MATCHING' ? 650 : 520)
    }
    ...
    await this.unlockDoor(target.slotNumber); onProgress?.('SUCCESS')
  ...
  async registerBiometric(type, employee, onProgress) {
    ensureMockData()
    for (const stage of ['PREPARING', 'COLLECTING']) { onProgress?.(stage); await wait(620) }
  ...
  async searchEmployees(query = '') {
    await wait(180); ensureMockData()
    const value = String(query).trim().toLowerCase()
    const list = !value ? appState.employees : appState.employees.filter((item) => `${item.employeeName} ${item.employeeId} ${item.employeeCode}`.toLowerCase().includes(value))
    return JSON.parse(JSON.stringify(list))
  },
  async deleteEmployee(id) {
    await wait(300); ensureMockData()
    const index = appState.employees.findIndex((item) => item.id === id || item.employeeId === id)
    if (index < 0) throw new Error('人员不存在')
    appState.employees.splice(index, 1); return true
  },
  async getHistory() { await wait(180); return JSON.parse(JSON.stringify(appState.history)) },
  async getUpgradeFiles() { await wait(220); return JSON.parse(JSON.stringify(upgradeFiles)) },
  async startUpgrade(fileId, onProgress) {
    for (const value of [8, 24, 48, 73, 100]) { await wait(380); onProgress?.(value) }
